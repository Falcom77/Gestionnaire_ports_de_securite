...
function App() {
  ...
  const exportFullCSV = async () => {
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'pfsense_export_complet.xlsx');
      document.body.appendChild(link);
      link.click();
      link.remove();
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className={`min-h-screen flex items-center justify-center ${darkMode ? 'bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900' : 'bg-gradient-to-br from-gray-100 via-gray-150 to-gray-200'}`} style={!darkMode ? {background: 'linear-gradient(135deg, #e5e7eb 0%, #d1d5db 50%, #cbd5e1 100%)'} : {}}>
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900' : ''}`} style={!darkMode ? {background: 'linear-gradient(135deg, #e5e7eb 0%, #d1d5db 50%, #cbd5e1 100%)'} : {}}>
      {/* Header */}
      <header className={`backdrop-blur-lg border-b sticky top-0 z-40 ${darkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-gray-200/90 border-gray-400'}`}>
                    ...
                    sortedDevices.map((device, idx) => (
                      <tr 
                        ...
                        className={`transition-colors ${
                          darkMode 
                            ? `hover:bg-gray-700/50 ${idx % 2 === 0 ? 'bg-gray-800/30' : 'bg-gray-800/10'}` 
                            : `hover:bg-gray-300/50 ${idx % 2 === 0 ? 'bg-gray-200/60' : 'bg-gray-100'}`
                        }`}
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-2 justify-center">
                            <button
                              ...
                              className={`p-2 rounded-lg transition-all ${darkMode ? 'bg-blue-500/20 text-blue-400 hover:bg-blue-500/30' : 'bg-blue-100 text-blue-700 hover:bg-blue-200'}`}
                              title="Modifier le périphérique"
                            >
                              <Edit className="w-6 h-6" />
                            </button>
                            <button
                              ...
                              className={`p-2 rounded-lg transition-all ${darkMode ? 'bg-red-500/20 text-red-400 hover:bg-red-500/30' : 'bg-red-100 text-red-700 hover:bg-red-200'}`}
                              title="Supprimer le périphérique"
                            >
                              <Trash2 className="w-6 h-6" />
                            </button>
                          </div>
